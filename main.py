...
# 2.58$ (минус 3% комиссия ~0.08$ = тебе придет ~2.50$)
PRICE_USD = 0.00
...
async def check_payment(callback: CallbackQuery):
    ...
    # Библиотека возвращает список или объект, проверяем статус
    is_paid = False
    ...
    if is_paid:
        ...
        await callback.answer("❌ Оплата не найдена. Подождите и попробуйте снова.", show_alert=True)
...
async def catch_sms(message: Message):
    ...
    if req and message.text.isdigit():
        await message.answer("⏳ Код принят. Ожидайте завершения.")
...
async def finish_job(callback: CallbackQuery):
    ...
    for req in pending_requests:
        if req["id"] == rid:
            ...
            await callback.message.edit_text(f"🏁 Заявка #{rid} готова.")
            break
